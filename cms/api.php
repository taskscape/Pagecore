<?php
/**
 * CMS JSON API. User-facing responses remain English to avoid mixed CMS locales.
 *
...
function cms_utf8_or_fail() {
    foreach (func_get_args() as $s) {
        if ($s !== '' && !preg_match('~~u', $s)) {
            cms_fail('Invalid character encoding (UTF-8 is required).');
        }
    }
}
...
/** Snapshot a source file plus every artifact regenerated from it. */
function cms_mutation_snapshot(array $sourcePaths) {
    $snapshot = cms_file_snapshot(array_merge($sourcePaths, cms_generated_artifact_paths()));
    if ($snapshot === null) { cms_fail('Could not prepare the content operation.', 500); }
    return $snapshot;
}

/** Require all derived artifacts to commit, otherwise restore the full operation. */
function cms_finish_indexed_mutation(array $snapshot) {
    $result = cms_regenerate_indexes();
    if (!is_array($result) || empty($result['ok'])) {
        $restored = cms_restore_file_snapshot($snapshot);
        error_log('CMS: content mutation rolled back after derived artifact failure; rollback ' . ($restored ? 'succeeded' : 'failed'));
        cms_fail('The content operation could not be completed. No changes were published.', 500);
    }
    return $result;
}
...
function cms_post_meta_from_request(array $meta, $strict) {
    ...
    if ($strict && $img !== '' && !preg_match('~^(/|https?://)[^\s"<>]+$~', $img)) {
        cms_fail('Image must be a path beginning with / or an http(s) URL.');
    }
    $meta['title'] = $title;
    $meta['date'] = $date;
    $meta['category'] = $cat;
...
function cms_preview_page($key, $kind, array $payload) {
    ...
    echo '<style nonce="' . htmlspecialchars(cms_csp_nonce(), ENT_QUOTES, 'UTF-8') . '">body{margin:0;background:#faf8f3;color:#2b2620;font:16px/1.6 "Open Sans",-apple-system,"Segoe UI",Roboto,Arial,sans-serif}'
    ...
    echo '<p class="cms-preview-note">Preview of a saved draft. This page is visible only while logged in to the CMS.</p>';
    if ($kind === 'post') {
...
$actionHandlers = array(
    ...
    if (empty($_FILES['file']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
        cms_fail('No file uploaded.');
    }
    $f = $_FILES['file'];
    if ($f['error'] !== UPLOAD_ERR_OK) { cms_fail('Upload failed (code ' . $f['error'] . ').'); }
