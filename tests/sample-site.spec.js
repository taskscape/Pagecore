...
test('editor can see the installed Pagecore version', async ({ page }) => {
  ...
  await expect(page.locator('.cms-toolbar')).toContainText('Pagecore 2.38.0');

  const version = await page.request.get('/cms/api.php?action=version');
  expect(version.ok()).toBeTruthy();
  expect((await version.json()).version).toBe('2.38.0');

  await page.goto('/cms/content.php');
  await expect(page.getByText('Pagecore 2.38.0')).toBeVisible();
});
